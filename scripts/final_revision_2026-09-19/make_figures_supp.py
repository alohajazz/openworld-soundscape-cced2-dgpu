...
# ---- S1 (needs the window-level score arrays) ------------------------------------------------------------------
S1_NPZ = ART / "ood/supp_fig_s1_scores_stage2.npz"
s = np.load(S1_NPZ) if S1_NPZ.exists() else None
if s is None: print("SuppFigS1 skipped: window-level score arrays are not available")
summ = json.loads((ART / "ood/supp_fig_s1_summary_stage2.json").read_text())
fig, axes = plt.subplots(1, 2, figsize=(13, 5.2), dpi=100)
for ax, (ind, ood, title, au) in zip(axes, () if s is None else ((s["beats_ind"], s["beats_ood"], "BEATs+DAPT (this work)", summ["beats"]["auroc"]),
...
if s is not None: fig.tight_layout(); fig.savefig(OUT / "SuppFigS1.png")
plt.close(fig)

# ---- S2 (area-proportional two-set Venn) and S3 (scatter) ---------------------------------------------------------
S3_NPZ = ART / "hiceas/supp_fig_s2_s3_data_stage2.npz"
d = np.load(S3_NPZ) if S3_NPZ.exists() else None
...
assert shared + ko == n_sel == shared + co and (d is None or len(d["shared"]) == shared)
...
if d is None: print("SuppFigS3 skipped: window-level score arrays are not available")
fig, ax = plt.subplots(figsize=(5.4, 4.5), dpi=200)
if d is not None: ax.scatter(d["dk_z"], d["dm_z"], s=1, color="#cbd5e1", alpha=0.35, linewidths=0, label=f"All windows (n={len(d['dk_z']):,})", rasterized=True)
for idx, col, lab in () if d is None else ((d["shared"], "#7c3aed", f"Shared (n={shared})"), (d["knn_only"], "#3b82f6", f"kNN_z only (n={ko})"), (d["cced2_only"], "#ef4444", f"CCED2 only (n={co})")):
...
if d is not None:
    ...
    fig.tight_layout(); fig.savefig(OUT / "SuppFigS3.png")
plt.close(fig)
