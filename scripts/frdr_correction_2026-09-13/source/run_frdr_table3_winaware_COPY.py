...
OUT_CSV = Path(_os.environ.get("T3_OUT", "/workspace/scripts/audit_2026-07-30/t3_rerun/frdr_table3_RERUN.csv"))
OUT_CSV.parent.mkdir(parents=True, exist_ok=True)

# 3 BEATs+DAPT encoders + Perch
BEATs_dirs = [
    (_os.environ.get("T3_NAME","BEATs+DAPT_fulldata"), _os.environ.get("T3_EMB","/workspace/embeddings/frdr_fulldata_winaware")),
]
PERCH_DIR = "/workspace/embeddings/perch_frdr_hop2s"

# InD references — encoder-paired
# 2026-07-31 コピー。原本 = scripts/winaware_2026-05-09/run_frdr_table3_winaware.py（無変更）。
# 環境変数で encoder と InD 参照のみ差し替え可能にした。他は一切変えていない。
_REF = _os.environ.get("T3_REF", "")
BEATs_KNN = (_REF + "/knn_cced2.pkl") if _REF else "/workspace/embeddings/known56/models/knn_dapt.pkl"
BEATs_MAHA = (_REF + "/maha_cced2.pkl") if _REF else "/workspace/embeddings/known56/models/maha_dapt.pkl"
BEATs_NORM = (_REF + "/cced2_norm.json") if _REF else "/workspace/embeddings/cced2_norm.json"
...
gt_by = complete_annotations(m, gt_by)
