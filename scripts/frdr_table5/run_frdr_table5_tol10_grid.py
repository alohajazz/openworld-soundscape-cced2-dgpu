...
Q_GRID = np.unique(np.concatenate([
    np.linspace(0.001, 0.10, 200, endpoint=False),
    np.linspace(0.10, 0.90, 800, endpoint=False),
    np.linspace(0.90, 0.9999, 1200)
]))
...
def extract_events(centers: np.ndarray, scores: np.ndarray, theta: float) -> list[float]:
    hi = scores >= theta
    n = len(scores)
    out: list[float] = []
    i = 0
...
def pick_best_theta(score_pf, cache, gt_by_file, hours):
    ...
    for q in Q_GRID:
        ...
        # 目的：FP/hをターゲットに近づける。タイなら recall が高い方。
        diff = abs(fp_h - TARGET_EVENT_FP_PER_HOUR)
        cand = (diff, -recall, theta, q, fp_h, pred_total, TP, FP, FN)
        if (best is None) or (cand < best):
            best = cand
    diff, neg_recall, theta, q, fp_h, pred_total, TP, FP, FN = best
    return dict(theta=theta, quantile=q, fp_h=fp_h, recall=-neg_recall,
                pred_events=pred_total, TP=TP, FP=FP, FN=FN, diff=diff)

def run_one(name, raw_score, m, gt_by_file, hours, hop, windows_per_hour):
    score_pf = preprocess_per_file(m, raw_score)
    cache = build_cache(m, score_pf)
    picked = pick_best_theta(score_pf, cache, gt_by_file, hours)
    return {
        "score": name,
        "tol_sec": TOL_SEC,
...
print(out[["score","theta","FP_per_hour","Recall","Pred_events","GT_events","TP","FP","FN","abs_diff_to_target","quantile"]].to_string(index=False))
print("\n[OK] wrote:", OUT_CSV)
