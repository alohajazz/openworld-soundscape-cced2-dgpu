...
EMB_DIR  = Path("/workspace/embeddings/frdr_fulldata")
ANN_CSV  = FRDR_DIR / "raw/data/continuous/dataset_B/annotations_B_cont.csv"

KNN_PKL   = Path("/workspace/embeddings/cced2_fulldata/knn_cced2.pkl")
MAHA_PKL  = Path("/workspace/embeddings/cced2_fulldata/maha_cced2.pkl")
NORM_JSON = Path("/workspace/embeddings/cced2_fulldata/cced2_norm.json")
...
OUT_SUMMARY = FRDR_DIR / f"table5_frdr_fulldata_tol{int(TOL_SEC)}_interp_at_fp{int(TARGET_FP_H)}.csv"
...
def interpolate_at_target(curve_fulldata_df, target_fp):
    # fp_hでソートして、targetを挟む2点を探す
    c = curve_fulldata_df.sort_values("FP_per_hour").reset_index(drop=True)
...
def run_curve(name, raw_score):
    ...
    dfc.to_csv(FRDR_DIR / f"curve_fulldata_{name}_tol{int(TOL_SEC)}.csv", index=False)
