...
> **Final revision (releases `v3.1.x-sr-minor-2026-09-19`; the manuscript cites the latest, `v3.1.6`).** The manuscript keeps its two-stage
...
## What can be run from this repository alone

No audio, embeddings or fitted reference models are distributed, so the analyses cannot be rerun end to end from
this repository alone. The following run on the public files and were tested on a fresh copy in a clean
environment; they need only `numpy`, `pandas`, `matplotlib`, `scipy` and `Pillow` from `requirements.txt`
(no PyTorch):

```bash
python scripts/verify_minor_revision_artifacts.py                          # hash/schema check of frozen aggregates
python scripts/final_revision_2026-09-19/table1_seed_summary.py paper_artifacts/final_revision_2026-09-19/table1/summary.csv
python scripts/final_revision_2026-09-19/table3_interpolate_fph10.py      # Table 3 from the published threshold sweep
python scripts/final_revision_2026-09-19/make_figures_main.py             # Fig. 3 and Fig. 4 (same values; PNG bytes depend on the matplotlib version)
python scripts/final_revision_2026-09-19/make_figures_supp.py             # Supplementary Fig. S2 and S4; S1 and S3 are skipped
```

What a full rerun needs, by result:

| Results | Additionally required |
|---|---|
| Promoter row of Table 2, the 300-label result, Table 4, cross-day results, Fig. 4 | FRDR / HICEAS audio from their providers, the encoders from Hugging Face, the HICEAS manifests in `paper_artifacts/minor_revision_2026-09/manifests/`, and a user-built FRDR manifest; embeddings must be re-extracted (`scripts/winaware_2026-05-09/dump_winaware_cached.py`) |
| Quiet, Union and Fusion rows of Table 2, Table 3, Supplementary Tables S3-S4, Supplementary Figs S1-S3 | all of the above **and** the CCED2 reference fitted on the non-public 56-class validation embeddings; these results cannot be regenerated by third parties |
| Table 1 | the non-public 56-class dataset |

### Paths and names from the original environment

Paths beginning with `/workspace/` in archived scripts are paths inside the container in which the analyses were
run; replace them with your own paths when adapting a script. In release `v3.1.4` the host and container names of
that environment were replaced by neutral wording or placeholders in a usage note of `frdr_label_efficiency_param.py`
(three archived copies) and in the `invocation` field of
`paper_artifacts/frdr_correction_2026-09-13/numerical_acceptance_receipt.json`. No code line and no result changed.
The package manifests list the files as distributed; the `source_sha256` values in the execution receipts refer to
the files as executed, before this replacement.
...
Obtain the corrected Stage 1 encoder and head from the immutable Hugging Face revision
in `MODEL_AVAILABILITY.json` (tag `v3.0.3-sr-minor-2026-09-13`); the Stage 2 encoder used for the FRDR and
HICEAS results of the final revision (`BEATs_DAPT_MAM_fixed_palaoa_step6385.pt`) is in the same Hugging Face
repository under the tags `v3.1.x-sr-minor-2026-09-19`. Use the exact
...
> **Superseded for the final revision.** The values and selection rules in this subsection describe the Stage 1
> record of `v3.0.4`/`v3.0.5`. The final-revision values, files and rules are in
> `paper_artifacts/final_revision_2026-09-19/README.md`. In the final revision the best Fusion weight is alpha = 0.0.
...
> **Superseded for the final revision.** The values and selection rules in this subsection describe the Stage 1
> record of `v3.0.4`/`v3.0.5`. The final-revision values, files and rules are in
> `paper_artifacts/final_revision_2026-09-19/README.md`. Table 3 of the final revision is interpolated
> linearly at FP/h = 10 (`scripts/final_revision_2026-09-19/table3_interpolate_fph10.py`), as its table note states.
...
> **Superseded for the final revision.** The values and selection rules in this subsection describe the Stage 1
> record of `v3.0.4`/`v3.0.5`. The final-revision values, files and rules are in
> `paper_artifacts/final_revision_2026-09-19/README.md`. Table 4 of the final revision is `hiceas/table4_stage2.json` there.
...
original execution environment and should be adapted when rerunning elsewhere.
